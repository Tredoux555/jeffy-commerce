...
export function SocialShare({ url, title, price, image }: SocialShareProps) {
  ...
  const shareText = price 
    ? `Check out ${title} for only ${formatCurrency(price)} on Jeffy! 🔥`
    : `Check out ${title} on Jeffy!`;
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-lg border p-3 z-50 w-48">
            <p className="text-xs text-gray-500 mb-2">Share to</p>
            
            <button
              onClick={shareToWhatsApp}
              className="flex items-center gap-3 w-full p-2 hover:bg-gray-50 rounded-lg transition text-left"
            >
