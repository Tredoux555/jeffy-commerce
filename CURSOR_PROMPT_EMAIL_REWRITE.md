# CURSOR PROMPT — Zone Partner Email Rewrite

## WHAT TO DO

Replace the Zone Partner confirmation email in `/src/app/api/zone-partners/route.ts` with a deeply inspirational email that conveys the full mission.

## FIND THIS CODE (around lines 208-280)

Look for this section that starts with:
```javascript
...
## REPLACE THE ENTIRE EMAIL SEND BLOCK WITH THIS:
...
1. The `firstName` variable is already defined earlier in the code (`const firstName = name.split(' ')[0];`)
2. The `zone_id` variable is already available from the request body
3. Keep the admin notification email unchanged (the one that sends to tredoux@gmail.com)
4. This only replaces the applicant confirmation email, not the waitlist email

## AFTER IMPLEMENTATION

1. Commit: `git add . && git commit -m "Rewrite Zone Partner email - full mission, family story, schools vision"`
2. Push: `git push origin main`
3. Railway will auto-deploy
4. Test with a new application at jeffy.co.za/partner/apply
